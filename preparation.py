...
def float_range(value):
  ...
  # Parse arguments
def Parse():
  parser = argparse.ArgumentParser(prog='separate.py', description='Preprocess and eparate the dataset into training dataset and validation dataset')
  ...
  parser.add_argument('--preprocessing', choices=['none', 'std', 'norm'], default='std',
...
def Preprocessing(dataset: pd.DataFrame, path_dataset: str, preprocess_mode: str) -> pd.DataFrame :
  ...
  if (preprocess_file == "none"):
    ...
    for column in data.columns[1:]:
      preprocess_file.write(f"{column},{data[column].mean()},{data[column].std()}\n")
      data[column] = (data[column] - data[column].mean()) / data[column].std()
  elif (preprocess_mode == "norm"):
    preprocess_file.write("column,min,max\n")
    for column in data.columns[1:]:
      preprocess_file.write(f"{column},{data[column].min()},{data[column].max()}\n")
